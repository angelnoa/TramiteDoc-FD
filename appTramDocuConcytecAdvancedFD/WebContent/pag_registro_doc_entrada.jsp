...
<script type="text/javascript" src="alertas/js/alertify.min.js"></script>
...
$(document).ready(function(){
	...
	window.alert = function(message){
	    $(document.createElement('div'))
	        .attr({title: '**Sistema de Tramite Documentario**', 'class': 'alert'})
	        .html(message)
	        .dialog({
...
$(document).ready(function(){
	...
	$(function() {
	    var availableTags = [
	      "gmail.com",
	      "hotmail.com",
	      "yahoo.com","concytec.gob.pe"
	    ];
	    $( "#dominio_correo" ).autocomplete({
...
$(function() {
  ...
  //      this.value = terms.join( ", " );
          this.value = terms;
          $("#dirigido_a").val(this.value);   
          //alert($("#dirigido_a").val());
          return false;
        }
      });
  });
...
function showDirgido(){
	timer = setTimeout('refrescaDirigido()', 500);
}

function refrescaDirigido(){
	var value= $("#dirigido").val();
	$("#dirigido_a").val(value);
	//alert(value);
}
...
function refresca(){
	...
	function buscar_persona(){
			...
			strURL += "&dirigido_a="+ls_dirigido_a;
...
function AgregarArchivos(){
	...
	if(rb_seleccion=='E'){
		...
		dirigido = document.getElementById("personas").value;
	}
	//alert(destinatario);
	//alert(dirigido);
	...
	+"&tipo="+tipo+"&operacionper="+operacionper+"&dirigido="+dirigido+"&dirigido_a="+ls_dirigido_a;
...
function MensajeRegistro(operacion,nro_registro,nro_expediente,clave){
		//var strURL="ValidaPaginas.do?tipo=mensajerecepcion";
 		//window.open(strURL,"","HEIGHT=140,WIDTH=500,scrollbars=no");
 		
 	var v=operacion;
	if(v==1){	
		//alert('<p align="left"><font color="red"><strong>REGISTRO OK :</strong></font></p><p align="left"><font color="red"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+nro_registro+'</strong></font></p><p align="left"><font color="red"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+nro_expediente+'</strong></font></p>');
		var nro_documento_ls=nro_registro.substring(nro_registro.indexOf(':')+1,nro_registro.length);
		var nro_expediente_ls=nro_expediente.substring(nro_expediente.indexOf(':')+1,nro_expediente.length);
		alert('<p align="left"><font color="red"><strong>REGISTRO OK :</strong></font></p><p align="left"><font color="red"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+nro_registro+'</strong></font></p><p align="left"><font color="red"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+nro_expediente+'</strong></font></p><p align="left"><font color="red"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+clave+'</strong></font></p><br><p align="left"><font color="blue"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="ValidaPaginas.do?tipo=verhojaruta&codigo_documento='+nro_documento_ls+'&codigo_expediente='+nro_expediente_ls+'"  target="_blank" title="Click para imprimir la Hoja de Ruta" >Imprimir Hoja de ruta</a></strong></font></p>');
	}
	if(v==2){	
		var nro_documento_ls=nro_registro.substring(nro_registro.indexOf(':')+1,nro_registro.length);
		var nro_expediente_ls=nro_expediente.substring(nro_expediente.indexOf(':')+1,nro_expediente.length);
...
function HabilitaText()
	...
	function Validar(){
		...
		//alert("<strong>hola</strong><br>hola");
			...
			if(rb_seleccion=='E'){
				...
				//alert(dirigido+" "+ $("#dirigido_a").val());
				if(dirigido==$("#dirigido_a").val()){
					//alert("es igual");
				}else{
					alert("Ingrese un nombre valido en Dirigido a.");
					document.form_datos.dirigido.focus();
					document.form_datos.cmb_grabar.value='Registrar';
	                document.form_datos.cmb_grabar.disabled=false;
					return false;  
		...
		if(rb_seleccion=='E'){
			codigo_intitucion = $("#codigo_intitucion").val();
			
			correo_electronico = document.getElementById("correo_electronico").value; 
	        dominio_correo = document.getElementById("dominio_correo").value; 
		...
		if(rb_seleccion=='E'){
			...
			"&correo_electronico="+correo_electronico+"&dominio_correo="+dominio_correo;
		}else{
			document.form_datos.action="MantDocumento.do?operacion=N"+"&medio="+medio
			+"&codigo_tipo="+codigo_tipo+"&codigo_oficina="+codigo_oficina+"&destinatario="+destinatario+"&codigo_solicitud="+
			codigo_solicitud+"&numero_documento="+numero_documento+"&fecha_registro="+fecha_registro+"&hora="+hora+"&folios_documento="+folios_documento+
...
<BODY bgColor="#F0F5FB" bottommargin="0" leftmargin="0" marginheight="0"
	marginwidth="0" rightmargin="0" topmargin="0"
	onload="MensajeRegistro('<c:out value='${operacionpopup}'/>','<c:out value='${mensaje_reg}'/>','<c:out value='${mensaje_exp}'/>','<c:out value='${mensaje_clave}'/>'); <c:set var="operacionpopup" scope="session" value="0"/> ">
	...
	<div class="derivaconta">
		...
		<c:choose>
			<c:when test='${operacionpopup ==  "X"}'>
				<SCRIPT language="javascript">
				// MensajeRegistro('<c:out value='${mensaje_reg}'/>','<c:out value='${mensaje_exp}'/>');
				</SCRIPT>
		...
		<form id="form_datos" name="form_datos" method="post" action=""
			...
			<INPUT type="hidden" id="operacion" name="operacion"
				value="<c:out value='${operacion}'/>"> <INPUT type="hidden"
				id="pertenece_a" name="pertenece_a"
				value="<c:out value='${pertenece_a}'/>"> <INPUT
				type="hidden" id="seleccions" name="seleccions"
				value="<c:out value='${rb_seleccion}'/>"> <INPUT
				type="hidden" id="dirigido_a" name="dirigido_a"
				value="<c:out value='${ls_dirigido_a}'/>">

			<div id="break"></div>
			<!-- <div id="ventana_flotante" class="rounded"></div>-->
			<div id="contactderiva" class="rounded">
				...
				<c:choose>
					<c:when test='${codigo_oficina ==  "1"}'>
						<div id="TDHeadCab">
							...
							<div id="botoninvisibleAnular" title="Modificar Registro"
								style="width: 80px; height: 20px; position: absolute; top: 6px; padding: 3px 10px; text-align: center; FONT-SIZE: 8pt; font-weight: normal; left: 86%; color: #003366; line-height: 20px;">
						...
						<div id="TDHeadCabRadios">
							...
							<input type="hidden" name="rb_seleccion" id="rb_seleccion"
								value="<c:out value='${rb_seleccion}'/>" />
							<c:choose>
								<c:when test="${rb_seleccion=='I'}">
									<INPUT name="rbuton" type="radio" value="E"
										onClick="javascript:cambiaCheck('E',this.form);">
					  Externo&nbsp;
					  <INPUT name="rbuton" type="radio" value="I" checked
										onClick="javascript:cambiaCheck('I',this.form);">
					                Interno				
					</c:when>
								<c:otherwise>
									<INPUT name="rbuton" type="radio" value="E" checked
										onClick="javascript:cambiaCheck('E',this.form);">
					  Externo&nbsp;
					  <INPUT name="rbuton" type="radio" value="I"
										onClick="javascript:cambiaCheck('I',this.form);">Interno				
					</c:otherwise>
							</c:choose>
						...
						<div id="left" class="columna-left2">
							...
							<p>
								<label>Medio:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<select id="medio" name="medio" class="input-medium"
								...
								</select>&nbsp;&nbsp;<A href="javascript:ayuda_doc_pend()">Documentos
									Pendientes</A>
							...
							<p>
								...
								<select id="codigo_tipo" name="codigo_tipo" class="input-medium">
									...
									<c:choose>
										<c:when test='${not empty rs_tipodoc}'>
							...
							<p>
								...
								<INPUT class="input" id="numero_documento" maxLength="40"
									size="40" name="numero_documento"
									style="text-transform: uppercase;"
									onkeyup="javascript:this.value=this.value.toUpperCase();"
									value="<c:out value='${numero_documento}'/>">
							</p>
							<p>
								<label>Fecha
									Doc.:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label> <INPUT
									class="input-medium" onKeyPress="validarCaracterFecha(this);"
									id="fecha_registro" style="width: 90px;" maxLength="10"
									size="11" name="fecha_registro"
							...
							<p>
								<label>Hora:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<INPUT class="input-medium" style="width: 90px;" id="hora"
									maxLength="8" size="9" name="hora"
									value="<c:out value='${hora}'/>"> &nbsp;&nbsp;&nbsp; <label>Folios:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<INPUT class="input-medium" id="folios_documento"
									style="width: 68px;" maxLength="3" size="3"
									name="folios_documento"
									onKeyPress="return validatecla('enteros', this);"
									value="<c:out value='${folios_documento}'/>">
							</p>

							<p>
								<label>Solicitud:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<select id="codigo_solicitud" name="codigo_solicitud"
									class="input">
									<OPTION value="0" selected>-- Selecc Uno --</OPTION>
									<c:choose>
										<c:when test='${not empty rs_solicitud}'>
											<c:forEach items='${rs_solicitud}' var='pa'>
												<option value=<c:out value='${pa.codigo_solicitud}'/>>
													<c:out value='${pa.nombre_solicitud}' />
												</option>
											</c:forEach>
										</c:when>
									</c:choose>
							...
							<p>
								<label>Asunto:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<TEXTAREA id="asunto_documento" class="textarea"
									style="WIDTH: 330px; HEIGHT: 70px" name="asunto_documento"
									rows="4" cols="71"><c:out
										value='${asunto_documento}' /></TEXTAREA>
							</p>
							<p>
								<label>Observaci&oacute;n:</label>
								<TEXTAREA id="observa_documento" class="textarea"
									style="WIDTH: 330px; HEIGHT: 70px" name="observa_documento"
									rows="4" cols="50"><c:out
										value='${observa_documento}' /></TEXTAREA>
							</p>

							<p>
								<label>Enviar Clave <br>a
									E-mail:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</label> <INPUT class="input-medium" id="correo_electronico"
									maxLength="40" size="40" name="correo_electronico" value="">
								<label>@</label> <INPUT class="input-medium" id="dominio_correo"
									size="10" name="dominio_correo"
									onkeypress="return validatecla('texto',this)" />
							</p>
						...
						<div id="rigth" class="columna-rigth3">
							<p>
									...
									<c:when test='${codigo_oficina ==  "1"}'>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input" maxLength="40" size="40"
													id="desc_origen" name="desc_origen" readonly="true"
													value="<c:out value='${descripcion_persona}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
													name="origen_documento" id="origen_documento"
													value='<c:out value='${codigo}'/>'>
												<INPUT type="hidden" maxLength="6" size="6"
													name="tipo_persona" id="tipo_persona"
													value='<c:out value='${tipopersona}'/>'>
											</c:when>
											<c:otherwise>
												<INPUT class="input" maxLength="40" size="40"
													name="desc_origen" id="desc_origen" readonly="true"
													value="<c:out value='${descripcion_persona}'/>">
											</c:otherwise>
										</c:choose>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input" maxLength="40" size="40"
													id="desc_origen" name="desc_origen" readonly="true"
													value="<c:out value='${desc_origen}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
													name="tipo_persona" id="tipo_persona"
													value="<c:out value='${tipo_persona}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
													name="origen_documento" id="origen_documento"
													value="<c:out value='${origen_documento}'/>">
											</c:when>
											<c:otherwise>
												<INPUT class="txt" maxLength="40" size="40"
													name="desc_origen" id="desc_origen" readonly="true"
													value="<c:out value='${desc_origen}'/>">
											</c:otherwise>
										</c:choose>
								...
								&nbsp; <A href="javascript:buscar_persona()"><IMG
									style="WIDTH: 20px; HEIGHT: 14px" height="20"
									src="img/flechadwnazul_on.gif" width="20" border="0"
									title="Seleccionar Procedencia"></A>
							</p>
							<p>
								<label>Tipo:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<c:choose>
									<c:when test='${codigo_oficina ==  "1"}'>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input-medium" id="naturaleza_documento"
													maxLength="40" size="20" name="naturaleza_documento"
													readonly="readonly" value='<c:out value='${tipopersona}'/>'>
											</c:when>
											<c:otherwise>
												<INPUT class="input-medium" type="hidden"
													id="naturaleza_documento" maxLength="40" size="20"
													name="naturaleza_documento" readonly="readonly"
													value='<c:out value='${tipopersona}'/>'>
												<INPUT class="input-medium" id="tipo" maxLength="40"
													size="20" name="tipo" readonly="readonly"
													value="<c:out value='${tipopersona}'/>">
											</c:otherwise>
										</c:choose>
									</c:when>
										...
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input-medium" id="tipo" maxLength="40"
													size="20" name="tipo" readonly="readonly"
													value="<c:out value='${tipo_persona}'/>">
												<INPUT class="txt" type="hidden" id="naturaleza_documento"
													maxLength="40" size="20" name="naturaleza_documento"
													readonly="readonly" value='<c:out value='${tipopersona}'/>'>
											</c:when>
											<c:otherwise>
							...
							<p>
								...
								<c:choose>
									<c:when test='${codigo_oficina ==  "1"}'>
										<INPUT class="input" id="firmadopor" maxLength="40" size="40"
											name="firmadopor" style="text-transform: uppercase;"
											onkeyup="javascript:this.value=this.value.toUpperCase();"
											value="<c:out value='${firmadopor}'/>">
										<INPUT id="INPUT4" type="hidden" maxLength="6" size="6"
											name="sle_codigo_contacto">
									</c:when>
									<c:otherwise>
										<INPUT class="input" id="firmadopor" maxLength="40" size="40"
											name="firmadopor" style="text-transform: uppercase;"
											onkeyup="javascript:this.value=this.value.toUpperCase();"
											value="<c:out value='${firmado_por}'/>">
										<INPUT id="INPUT4" type="hidden" maxLength="6" size="6"
											name="sle_codigo_contacto">
									</c:otherwise>
								</c:choose>
							...
							<p>
								<label>Institucion <br>Destino:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									...
									class="input-medium" onchange="limpiaCampos()">
									<OPTION value="0" selected>-- Selecc Uno --</OPTION>
							...
							<p>
								...
								<c:out value='${operacion_dir}' />
								<c:choose>
									<c:when test='${operacion_dir == "D"}'>

										<INPUT class="input" id="dirigido" maxLength="40" size="40"
											name="dirigido" value="<c:out value='${dirigido}'/>">
									</c:when>
									<c:otherwise>
										<INPUT class="input" id="dirigido" maxLength="40" size="40"
											name="dirigido" value="<c:out value='${dirigido}'/>">
							...
							<p>
								<label>Destino:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label> <select
									id="codigo_oficina" name="codigo_oficina" class="input"
							...
							<p>
								...
								<c:choose>
									...
									<c:otherwise>
										...
										<select id="personas" name="personas" class="input">
											<c:choose>
												<c:when test='${not empty rs_destinatario}'>
													<OPTION value="0">-- Selecc Uno --</OPTION>
							...
							<p>
								<label>Fecha de <br>Respuesta Legal:
								</label> <input type="checkbox" class="caja" name="cbo_fecharpta"
									id="cbo_fecharpta" value="checkbox"
									onClick="javascript:HabilitaText();"> &nbsp;<INPUT
									class="input-medium" style="width: 90px;"
									onKeyPress="validarCaracterFecha(this);" id="fecha_rpta"
									maxLength="10" size="11" name="fecha_rpta"> &nbsp; <a
									href="" id="lanzador1" style="display: none; width: 20;"><img
									src="img/cal.gif" alt="Fecha de Respuesta Legal (dd/mm/yyyy)"
									border="0"></a>
							</p>
							<p>
								<label>N&deg; de <br>Referencia:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</label> <INPUT class="input" id="numero_referencia" maxLength="10"
									size="10" name="numero_referencia" style="width: 90px;"
									onKeyPress="return validatecla('enteros', this);"
									value="<c:out value='${numero_referencia}'/>">
							</p>
							<p>
								<label>Adjuntar Doc:&nbsp;&nbsp;&nbsp;&nbsp;</label> <input
									type="button" id="btnUpload" class="button" name="btnUpload"
									value="Agregar Archivos" onClick="AgregarArchivos()"
									title="Agregar Archivos" class="boton">&nbsp;
								<c:choose>
									<c:when
										test='${operacion ==  "T" && FFormMantDocumento.numeroArchivos > 0 }'>
					...
					<c:otherwise>
						...
						<div id="TDHeadCabRadios">
							<c:choose>
								<c:when test='${codigo_oficina ==  "1"}'>
									<INPUT id="rb_seleccion" type="radio" value="E"
										name="rb_seleccion" checked>
					      	Externo&nbsp;
					      	<INPUT id="rb_seleccion" type="radio" value="I"
										name="rb_seleccion">
				        	Interno				
			        </c:when>
								<c:otherwise>
									<INPUT id="rb_seleccion" type="radio" value="I"
										name="rb_seleccion" checked>Interno				
			        </c:otherwise>
							</c:choose>
						</div>
						<div id="Lineaform2"></div>
						...
						<div id="left" class="columna-left2">
							...
							<p>
								<label>Fecha
									Doc.:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label> <INPUT
									class="input-medium" style="width: 90px;"
									onKeyPress="validarCaracterFecha(this);" id="fecha_registro"
									maxLength="10" size="11" name="fecha_registro"
									value="<c:out value='${fecha}'/>"> &nbsp; <a href=""
									id="lanzador"><img src="img/cal.gif"
									alt="Fecha de nacimiento (dd/mm/yyyy)" border="0"></a>
							</p>
							<p>
								...
								<INPUT class="input-medium" style="width: 90px;" id="hora"
									maxLength="8" size="9" name="hora"
									value="<c:out value='${hora}'/>"> &nbsp;&nbsp;&nbsp; <label>Folios:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<INPUT class="input-medium" style="width: 68px"
									onKeyPress="return validatecla('enteros', this);"
									id="folios_documento" maxLength="3" size="3"
									name="folios_documento"
									value="<c:out value='${folios_documento}'/>">
							</p>
							...
							<p>
								<label>N&deg; de <br>Referencia:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</label> <input class="input" id="numero_referencia"
									style="width: 90px;"
									onKeyPress="return validatecla('enteros', this);"
									maxlength="10" size="10" name="numero_referencia"
									value="<c:out value='${numero_referencia}'/>">
							</p>
							<p>
								<label>Adjuntar Doc:&nbsp;&nbsp;&nbsp;&nbsp;</label> <input
							...
							<p>
								<label>Asunto:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
								<textarea id="asunto_documento"
									style="WIDTH: 330px; HEIGHT: 70px" class="textarea"
									name="asunto_documento" rows="2" cols="71"><c:out
										value='${asunto_documento}' /></textarea>
							</p>
							<p>
								<label>Observaci&oacute;n:</label>
								<textarea id="observa_documento"
									style="WIDTH: 330px; HEIGHT: 70px" class="textarea"
									name="observa_documento" rows="2" cols="50"><c:out
										value='${observa_documento}' /></textarea>
							</p>
							...
							<p>
								...
								<c:choose>
									<c:when test='${codigo_oficina ==  "1"}'>
										...
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input" style="width: 300px" maxLength="40"
													size="40" id="desc_origen" name="desc_origen"
													readonly="true"
													value="<c:out value='${descripcion_persona}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
													name="origen_documento" id="origen_documento"
				                ...
				                &nbsp;
								<A href="javascript:buscar_persona()"><IMG width="16px"
											height="16px" border="0" src="img/flechadwnazul_on.gif"
											title="Seleccionar Procedencia"></A>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input" style="width: 300px" maxLength="40"
													size="40" id="desc_origen" name="desc_origen"
													readonly="true" value="<c:out value='${desc_origen}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
													name="tipo_persona" id="tipo_persona"
													value="<c:out value='${tipo_persona}'/>">
												<INPUT type="hidden" maxLength="6" size="6"
											...
											<c:otherwise>
												<INPUT class="input" style="width: 300px" maxLength="40"
													size="40" name="desc_origen" id="desc_origen"
													readonly="true" value="<c:out value='${desc_origen}'/>">
											</c:otherwise>
										</c:choose>
									</c:otherwise>
									...
									<c:when test='${codigo_oficina ==  "1"}'>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												<INPUT class="input-medium" id="naturaleza_documento"
													maxLength="40" size="20" name="naturaleza_documento"
													readonly="true" value='<c:out value='${tipopersona}'/>'>
											</c:when>
											<c:otherwise>
												<INPUT class="txt" type="hidden" id="naturaleza_documento"
													maxLength="40" size="20" name="naturaleza_documento"
													readonly="true" value='<c:out value='${tipopersona}'/>'>
												<INPUT class="input-medium" id="tipo" maxLength="40"
													size="20" name="tipo" readonly="true"
													value="<c:out value='${tipopersona}'/>">
											</c:otherwise>
										</c:choose>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test='${operacion == "T" }'>
												...
												<INPUT class="input-medium" type="hidden"
													id="naturaleza_documento" maxLength="40" size="20"
													name="naturaleza_documento" readonly="true"
													value='<c:out value='${tipopersona}'/>'>
											</c:when>
											<c:otherwise>
												<INPUT class="input-medium" id="tipo" maxLength="40"
				    	...
				    	<p><label>Dirigido a:&nbsp;&nbsp;&nbsp;</label>
								...
								<c:choose>
									...
									<c:otherwise>
										<select id="personas" name="personas" class="input">
											...
											<c:choose>
												<c:when
													test='${not empty rs_destinatario}'>
													<OPTION value="0">-- Selecc Uno --</OPTION>
													<c:forEach items='${rs_destinatario}' var='pa'
														varStatus='rowCounter'>
														<c:choose>
							...
							<div id="scroll" class="input"
								style="WIDTH: 370px; height: 100px;">
								<table>

									<c:forEach items='${rs_oficina}' var='pa'>
										<tr>
											<td><input class="caja" type="checkbox"
												name="selectedItems"
												value="<c:out value='${pa.codigo_oficina}'/>"></td>
							...
							<p>
								<label><c:choose>
										<c:when test='${pertenece_a ==  "3"}'>Con Copia CONCYTEC</c:when>
										<c:otherwise>Con Copia FONDECYT</c:otherwise>
									</c:choose></label> <input id="chk_copia_other" checked="checked" class="caja"
									onClick="javascript:HabilitaCombo(0);" type="checkbox"
									name="chk_copia_other">
							</p>
							</div>
							<div id="scroll_2" class="input"
								style="WIDTH: 370px; height: 100px;display:none;">
								<table>

									<c:forEach items='${rs_oficina_other}' var='da'>
										<tr>
											<td><input class="caja" type="checkbox"
												name="selectedItems_other"
												value="<c:out value='${da.codigo_oficina}'/>"></td>
											<td class="texto_small"><c:out
													value='${da.siglas_oficina}' /> - <c:out
													value='${da.descripcion_oficina}' /></td>
										</tr>
									</c:forEach>
								</table>
							</div>
				...
				<div class="center-big">
					<INPUT class="button" id="cmb_grabar"
						onclick="this.disabled=true; this.value='REGISTRANDO...'; Validar()"
						type="button" value="Registrar">
				</div>
				<SCRIPT language="javascript">
			Calendar.setup({
 			...
 			Calendar.setup({
					...
					inicio();
					//alert("campo ->"+ $("#dirigido").val()+" "+"memoria ->"+ $("#dirigido_a").val());
					function inicio(){
					//var scroll_chks = document.getElementById('scroll');
					with(document.forms[0]){
							fecha_rpta.disabled = "true"; 
