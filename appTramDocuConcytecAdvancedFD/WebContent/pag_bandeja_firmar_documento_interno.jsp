		...
		$("#enviar-complemento").click(function () {
			...
			if(tipofirma==0){
				...
				if(tipofirma==1){
					...
					strURL = "ResultadoAJAXFD.do?codDoc="+codigo+"&codTip="+tipo+"&observacion="+observacion+"&tipoenvio="+tipoenvio;
					FirmaArchivo(strURL);
...
function FirmaArchivo(url_pdf){		
	$.ajax({
		type: "GET",
        url: url_pdf + "&tipo=copyArchivo",
...
//         	alert(data);
        	if (data != "" && data != "Error"){
                ...
                randonId = datetime.getTime();
                var possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
                for (var i = 0; i < 10; i++)
                    randonId += possible.charAt(Math.floor(Math.random() * possible.length));

                try {
                    if (typeof (chrome) !== 'undefined') {
                        randonId = "chr" + randonId;
                        foo(randonId);
                    }
                } catch (err) { }
                
    			var url = "AutenticarArchivoFD.do?nom_arch=" + data + "&randonid=" + randonId;
    			var left = (screen.width / 2) - (650 / 2);
    		    var top = (screen.height / 2) - (380 / 2);
    		    window.open(url, 'Firma Digital', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=' + 660 + ', height=' + 370 + ', top=' + top + ', left=' + left);
    		}else{
				alert("Error al preparar archivos");
			}
        }
    });
...
function getResponseChild(message) {
	if (message == "0"){
		alert("El documento ha sido firmado");
		
		
	}
}

function foo(nom_archivo) {
	...
	$.ajax({
        ...
        contentType: "application/x-www-form-urlencoded; charset=UTF-8",
        dataType: "text",
        data: { tipo : "leeR", nomarch: nom_archivo},
        	...
        	try {
                if (result.trim() != '') {
                	if (result.indexOf("0") > -1){
                		...
                		alert("Error al realizar la firma del documento");
                	}                    		
                } else {
                    setTimeout(function(){foo(nom_archivo);}, 3000);
                }
            } catch (e) {}
        }
    });
...
function mostrarlistasdebandejadocumentosinternos(valor){
	//var strURL = "ModificarDocumentoInterno.do?cod_doc_interno="+valor;
	//location=strURL;

        var horizontalPadding = 15;
        var verticalPadding = 15;
        refDialog = $('<iframe id="site" src="' + 'mostrarlistasdebandejadocumentosinternos.do?cambia=upload&codigo_documento_interno='+valor+ '" />').dialog({
...
function MostrarMensaje(mensaje,coDoc){
	...
	if(mensajez == 0){
		if(codocu == 0){
			strURL = "UtilidadActualizacionRegistroMovimiento.do?operacion=update";	
		}
...
function LlamarPuente(){
 ...
 function firmaMasivaIframeFirmaMasiva(){
	 //alert("hola");
	 var strURL = "CopiaMasivaPDFATemporal.do?pass=xxxxxxx&log=0021334";
		winPopup = window.open(strURL,"Firma_Digital","scrollbars=YES,resizable=YES,location=NO,WIDTH=730,HEIGHT=430");
 }
...
<div class="container">
  ...
  <c:otherwise>
  	...
  	<iframe id="site" src="mostrarlistasdebandejadocumentosinternos.do?cambia=firma" style="width:1000px; height: 800px; background-color: lightyellow; border: 0;" ></iframe>
