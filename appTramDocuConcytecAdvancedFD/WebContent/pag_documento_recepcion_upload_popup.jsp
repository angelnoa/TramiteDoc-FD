...
function Aceptar(){
	...
	document.form_datos.action='MantRecepcion.do'+'?operacion='+ls_operacion+'&tipo=uploadficheros'+'&codigo_documento='+ls_codigo_documento+'&codigo_expediente='+ls_codigo_expediente+'&secuencia_movimiento='+ls_secuencia_movimiento;
          ...
          <TABLE width="100%" cellpadding="0" cellspacing="0">
            <TR> 
              <TD width="978" height="49" valign="middle" bgcolor="#BAD0EB" class="titulolistado01" ><FONT size="2">&nbsp; 
                RECEPCIÓN DOCUMENTOS - ADJUNTAR ARCHIVOS
                </FONT></TD>
            </TR>
          </TABLE>
        ...
        <c:out value='${pr.nombre_archivo}'/> </td>
		   ...
		   <td align="center" valign="middle" class="Columna">   <c:choose>
		     <c:when test='${pr.nombre_archivo ==  ""}'>
		       ------		  </c:when>
		     <c:otherwise>
  <a target="_blank" href="uploadDocumentos/<c:out value='${pr.nombre_archivo}'/>">Ver Archivo</a>	</c:otherwise>
		            </c:choose>	  </td>
    <td align="center" valign="middle"><A id="A1" target="_parent" href="MantRecepcion.do?tipo=eliminar&id_det_upload=<c:out value='${pr.id_det_upload}'/>&codigo_documento=<c:out value='${codigo_documento}'/>">
