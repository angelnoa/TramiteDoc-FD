...
<script language="javascript">
	function abrirVentana(ventana)
	{
		...
		S.type = 'text/css';
		S.appendChild(document.createTextNode('body{overflow-x:hidden;}'));
	...
	function cerrarVentana()
	{
		...
		document.form_datos.bAceptar.blur();
	}
</script>
...
function ModificarEstado(nombre){
	...
	location = "AdjuntarDocumentoInternoT.do?operacion="+ls_operacion+"&nombre="+nombre;
	
}

function eliminar_archivo(id){
	if(confirm("Desea Eliminar este archivo?")){
		location = "Busquedas.do?tipo=eli_archivo_interno_especialt&id="+id;
	}
} 
		 ...
		 if (!permitida) { 
             ...
             return false;
	   			...
	   			var nombre_archivo = document.getElementById("fileupload").value;
	   		 	document.form_datos.action='AdjuntarDocumentoInternoEspecialT.do?operacion='+ls_operacion;
...
<BODY class="def"  leftMargin="0"  topMargin="0" rightMargin="0" bottomMargin="0" marginwidth="0" marginheight="0" bgcolor="#F0F5FB">
...
<div id="uploadcontainer">
    ...
    <td width="68" valign="top" background="img/fondoplomo8.gif"><!--DWLayoutEmptyCell-->&nbsp;</td>
      ...
      <c:forEach items='${FFormDocumentoInternoEspecialT.archivos}' var='ar' varStatus="c"> 
    ...
    <c:choose>
     <c:when test='${not empty rs_upload_doc_internos}'>
  	...
  	<a  target="_blank" href="VerArchivoPDFDocumentoInterno.do?filename=<c:out value='${pr.nombre_archivo_cifrado}'/>&fileroute=xipxx">Visualizar</a>
		             </td>
		             <c:choose>
	       <c:when test='${esApto == "1"}'>
	       <c:choose>
    ...
    <td align="center" valign="middle"><A id="A1" target="_parent" href="AdjuntarDocumentoInternoEspecialT.do?operacion=EDIF&nombre=<c:out value='${pr.nombre_archivo}'/>&cod_doc_interno=<c:out value='${pr.tipo_archivo}'/>">
      ...
      <td align="left" valign="middle" class="Columna"><c:out value="${ar.nombre_archivo}"></c:out></td>
		   ...
		   <td align="center" valign="middle" class="Columna"><a href="Busquedas.do?tipo=ver_archivo_interno_especialt&id=<c:out value='${c.count-1}'/>">Visualizar</a></td>
		   <c:choose>
	<c:when test='${esApto == "1"}'>
	<c:choose>
    ...
    <td align="center" valign="middle"><a href="javascript:eliminar_archivo(<c:out value='${c.count-1}'/>)"><img src="img/trash.png" border="0" height="24" alt="Eliminar Linea."></a></td>
     ...
     <!-- div class="containerlista">       
      <TABLE width="100%" border="0"  >
            ...
            <c:choose><c:when test='${operacioninterno == "O"}'>
				...
				<tr>
				  ...
				  <display:table name="sessionScope.listaDocumentosInternosFirmados" export="false" sort="list" id="b" pagesize="5"  class="simple" style="width: 100%">
